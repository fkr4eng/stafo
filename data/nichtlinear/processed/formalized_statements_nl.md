...
- Concepts in this snippet:
    ...
    - 'has solution'
- // snippet(141)
- // This snippet is the proof (Beweis) of the proposition "Adjungierte Variationsgleichung" (snippet 140): Z(t) := phi_{-t}'(phi_t(p)) is shown to satisfy Z-dot = -Z * f'(phi_t(p)), Z(0) = I, by differentiating the identity phi_{-t}(phi_t(x)) = x (valid since phi_t is a local diffeomorphism) with respect to x to obtain phi_{-t}'(phi_t(x)) * phi_t'(x) = I, then differentiating again with respect to t, evaluating at x = p, substituting the Variationsgleichung's ODE (snippet 136) for the time derivative of phi_t'(p), and right-multiplying by the inverse of phi_t'(p). As in the analogous proof of snippet 138 (Proof of Variationsgleichung), the setting is re-declared locally rather than referencing snippet 140's named instances directly, since named instances declared inside a formalized setting/premise/assertion block are locally scoped in the converter and cannot be referenced from a different, later complex statement (root cause documented for snippet 137). Reuses 'flow', 'is flow of', 'vector field', 'has differentiability class', 'Jacobian matrix', 'has Jacobian matrix', 'time derivative op', 'square matrix', 'identity matrix', 'initial value problem', 'solution', 'has solution', 'has proof' and the standard M/Rn/vector setup, all as in snippet 140's own proposition statement. No new class, property, relation or operator is introduced.
- There is an if-then-statement ('Proof of Adjungierte Variationsgleichung'):
    - full source code: Der Fluss~$\varphi_{t}$ ist ein lokaler Diffeomorphismus. Daher gilt $\varphi_{-t}(\varphi_{t}(x))=x$ für alle $x$ in der Umgebung des Anfangswertes~$p$. Differentiation nach~$x$ liefert \begin{equation} \varphi_{-t}^{\prime}(\varphi_{t}(x))\cdot\varphi_{t}^{\prime}(x)=I. \end{equation} Eine weitere Differentiation nach~$t$ und das Einsetzen $x=p$ führen auf \[ \frac{\partial}{\partial t}\left(\varphi_{-t}^{\prime}(\varphi_{t}(p))\right)\cdot\varphi_{t}^{\prime}(p)+\varphi_{-t}^{\prime}(\varphi_{t}(p)\cdot\underbrace{\frac{\partial}{\partial t}\left(\varphi_{t}^{\prime}(p)\right)}_{{\displaystyle (*)}}=0. \] Der Term~$(*)$ kann durch die rechte Seite der Variationsgleichung~(\ref{eq:Variationsgleichung}) ersetzt werden. Mit $Z(t)$ entsprechend Gl.~(\ref{eq:Loesung_Adj_Variationsgl}) erhält man \[ \dot{Z}(t)\cdot\varphi_{t}^{\prime}(p)+Z(t)\cdot f^{\prime}(\varphi_{t}(p))\cdot\varphi_{t}^{\prime}(p)=0. \] Multipliziert man diese Gleichung von rechts mit $(\varphi_{t}^{\prime}(p))^{-1}$, so erhält man die gewünschte Differentialgleichung~(\ref{eq:AdjVariationsgleichung}).
    - formalized setting:
        - 'n' is an instance of 'integer number'
        - 'Rn' is an instance of 'real vector space'
        - 'Rn' 'has dimension' 'n'
        ...
        - There is an equation:
            - full source code: 'Z'('t') == 'J_mt'('phi_t'('p'))
    - formalized assertion:
        - // Differentiating the identity phi_{-t}(phi_t(x)) = x (valid since phi_t is a local diffeomorphism) with respect to x gives phi_{-t}'(phi_t(x)) * phi_t'(x) = I; a further differentiation with respect to t, evaluated at x = p, combined with the Variationsgleichung's ODE for phi_t'(p) (snippet 136) and right-multiplication by the inverse of phi_t'(p), yields the ODE below for Z(t) = phi_{-t}'(phi_t(p))
        - There is a system of equations ('adjungierte variationsgleichung'):
            - There is an equation:
                - full source code: 'time derivative op'('Z')('t') == 0 - 'Z'('t') * 'Jf'('phi_t'('p'))
            - There is an equation:
        ...
        - 'Z' is secondary instance of 'solution'
        - 'adjungierte variationsgleichung' 'has solution' 'Z'
- 'Adjungierte Variationsgleichung' 'has proof' 'Proof of Adjungierte Variationsgleichung'
- Concepts in this snippet:
    - 'integer number'
    - 'real vector space'
    - 'has dimension'
