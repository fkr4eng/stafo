...
- There is a general statement:
    ...
    \end{multline}'
    - formalized setting:
        - // Not sure how to model the statement 'unter der Annahme an, dass sämtliche Anfangswerte Null sind'. It seems to be a condition for the equation to hold. However, it is not clear to me how to represent this condition.
    - formalized assertion:
        - There is an equation:
...
- // I am not sure how to model the relation between equation 'eq:DglAllgemeinLinearBildbereich', equation '\eqref{eq:DglAllgemeinLinear}' and equation '\eqref{eq:LaplaceDiffAllgemein}'. It seems that 'eq:DglAllgemeinLinearBildbereich' is the result of applying the Laplace transformation '\eqref{eq:LaplaceDiffAllgemein}' to the differential equation '\eqref{eq:DglAllgemeinLinear}'. However, it is not clear to me how to represent this.
