...
- There is a general statement:
    ...
    - formalized setting:
        - '\(f\)' is an instance of 'Funktion'
        - '\(F\)' is an instance of 'Funktion'
        - '\(F^\ast\)' is an instance of 'Funktion'
    - formalized assertion:
        - There is an equation:
            - full source code: '\(F^\ast(s) = \int\limits_0^\infty \diff{f}{t}(t)\mathrm{e}^{-st}\df t\)'
            - // I cannot generate the formalized left hand side and formalized right hand side because I do not know how to represent the integral.
        - There is an equation:
            - full source code: '\(F^\ast(s) = \left[ \mathrm{e}^{-st} f(t) \right]_{t=0}^{t=\infty} - \int\limits_0^\infty f(t)(-s\mathrm{e}^{-st})\df t\)'
            - // I cannot generate the formalized left hand side and formalized right hand side because I do not know how to represent the integral.
        - There is an equation:
            - full source code: '\(F^\ast(s)=-f(0) + s \int\limits_0^\infty f(t)\mathrm{e}^{-st}\df t\)'
            - // I cannot generate the formalized left hand side and formalized right hand side because I do not know how to represent the integral.
        - There is an equation:
            - full source code: '\(F^\ast(s)=-f(0) + sF(s)\)'
            - // I cannot generate the formalized left hand side and formalized right hand side because I do not know how to represent the function application.
        - There is an equation:
...
- // I am not sure how to model the term 'Partielle Integration liefert'. It seems to be a justification for the following equation. However, it is not clear to me how to represent this justification.
- // I am not sure how to model the term 'Wegen \eqref{eq:LaplaceHin} ergibt sich daraus'. It seems to be a justification for the following equation. However, it is not clear to me how to represent this justification.
