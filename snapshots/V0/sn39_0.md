...
- 'eq:DlgAllgemein' is associated to 'Beschreibungsform'
- // I am not sure how to model the term 'Beschreibungsform'. It seems to refer to the description of a system using a differential equation. However, it is not clear to me what this term represents exactly.
- There is an instance: '\(n\)'
- '\(n\)' is an instance of '\(\mathbb{N}\)'
- There is an instance: '\(m\)'
- '\(m\)' is an instance of '\(\mathbb{N}\)'
- There is an instance: '\(F\)'
- '\(F\)' is an instance of 'Funktion'
- '\(F\)' has domain '\(\mathbb{R}^{n+m+2}\)'
- '\(F\)' has range '\(\mathbb{R}\)'
- // I am not sure how to model the term 'Anfangsbedingungen'. It seems to refer to the initial conditions of a differential equation. However, it is not clear to me what this term represents exactly.
- // I am not sure how to model the equations '\(\nu(0) =: V_{00}\)', '\(\dot \nu(0) =: V_{01}\)', '\(\ldots\)', '\(\nu^{(n-1)}(0) =: V_{0n-1}\)'. They seem to define the initial conditions of a differential equation. However, it is not clear to me how to represent these equations.
- There is a property: 'Ordnung'
- 'Ordnung' has the alternative label 'order'
- 'Differenzialgleichung' has the property 'Ordnung'
- // I am not sure how to model the statement 'Den Parameter \(n\) bezeichnet man dabei als \emph{Ordnung} der Differenzialgleichung.' It seems to define the term 'Ordnung' for a 'Differenzialgleichung'. However, it is not clear to me how to represent this definition.
