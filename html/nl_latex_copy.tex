...
\setref{Jeder Vektor}{label:vector}~(\ref{eq:vektor-x})
lässt sich eindeutig als Linearkombination der \setref{Basisvektoren}{label:basis} darstellen:
\[
x=x_{1}e_{1}+\cdots+x_{n}e_{n}.
\]

Die \setref{\textbf{\em lineare Hülle}}{label:linear hull} (engl. \textbf{\em \setref{linear hull}{label:linear hull}, \setref{linear span}}{label:linear hull}}) von $r$ \setref{Vektoren}{label:vector} $v_{1},\ldots,v_{r}\in{\mathbb{R}}^{n}$ ist die \setref{Menge}{label:set} aller Linearkombinationen dieser \setref{Vektoren}{label:vector}:
\[
{\operatorname{\setref{span}{label:span}}}\left\{ v_{1},\ldots,v_{r}\right\} :=\left\{ \alpha_{1}v_{1}+\cdots+\alpha_{r}v_{r};\,\alpha_{1},\ldots,\alpha_{r}\in{\mathbb{R}}\right\} .
\]
...
Durch Verknüpfung von \setref{Elementen}{label:is element of} aus Primal- und \setref{Dualraum}{label:dual space} erhält man mit
\begin{equation}
...
\end{equation}
eine \setref{natürliche Paarung}{label:natural pairing}  $\left\langle \cdot,\cdot\right\rangle :({\mathbb{R}}^{n})^{*}\times{\mathbb{R}}^{n}\to{\mathbb{R}}$, die man auch als \setref{duale Paarung}{label:natural pairing}, \setref{inneres Produkt}{label:natural pairing} oder \setref{Kontraktion}{label:natural pairing} zwischen \setref{Kovektoren}{label:covector} und \setref{Vektoren}{label:vector} auffasst.
...
\setref{kanonisches Skalarprodukt}{label:canonical scalar product} verwendet~\cite{bishop1980}.
