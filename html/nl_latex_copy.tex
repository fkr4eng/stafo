...
Die Ableitung einer \setref{differenzierbaren vektoriellen Funktion}{label:differentiable vector-valued function} $F:\mathcal{M}\to{\mathbb{R}}^{m}$ ist die $m\times n$-Matrix
...
\end{equation}
{label:Jacobian matrix} bzw. \textbf{\em Differential} genannt wird. Die \setref{Jacobimatrix}{label:Jacobian matrix} eines \setref{Vektorfeldes}{label:vector field} $f:\mathcal{M}\to{\mathbb{R}}^{n}$ ist eine \setref{quadratische}{label:square matrix} $n\times n$-\setref{Matrix}{label:matrix}.
